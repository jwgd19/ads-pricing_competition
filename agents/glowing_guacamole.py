...
class Agent(object):
    def __init__(self, agent_number, params={}):
        ...
        self.prices_to_predict_0 = np.linspace(min(self.training_range['price_item_0']),
                                               max(self.training_range['price_item_0']), self.qcut)
        self.prices_to_predict_1 = np.linspace(min(self.training_range['price_item_1']),
                                               max(self.training_range['price_item_1']), self.qcut)
    ...
    def _process_last_sale(self, last_sale, profit_each_team):
        ...
        if not math.isnan(my_last_prices[0]):
            if not did_customer_buy_from_me and not did_customer_buy_from_opponent:
                ...
                if price0 < self.ub0 and self.ub0 > 10:
                    self.ub0 = price0
                if price1 < self.ub1 and self.ub1 > 10:
                    self.ub1 = price1

    def get_predict_model(self):
        oh = np.array(self.opponent_history[-50:])
        bh = np.array(self.behavior_history[-50:])
        ph = np.array(self.price_history[-51:len(self.price_history) - 1])
    ...
    def get_predict_model0(self):
        oh = np.array(self.opponent_history0[-50:])
        bh = np.array(self.behavior_history0[-50:])
        ph = np.array(self.price_history0[-51:len(self.price_history0) - 1])
        # ah = np.array(self.agent_history)
        X_train = np.column_stack([bh, ph])
        Y_train = oh.reshape(-1, 1)
        # print(X_train.shape)
    ...
    def get_predict_model1(self):
        oh = np.array(self.opponent_history1[-50:])
        bh = np.array(self.behavior_history1[-50:])
        ph = np.array(self.price_history1[-51:len(self.price_history1) - 1])
        # ah = np.array(self.agent_history)
        X_train = np.column_stack([bh, ph])
        Y_train = oh.reshape(-1, 1)
        # print(X_train.shape)
    ...
    def action(self, obs):
        ...
        if self.project_part == 1:
            ...
            # coefficient based on opponent's history and price
            consecutive_lose = 0
            lose_coef = 0.9
            if (last_sale[self.this_agent_number] == 0):
                consecutive_lose += 1
            else:
                consecutive_lose = 0
            ...
            # for every 100 rounds, update the model
            ...
            if (self.model != []):
                ...
                random_coef = random.uniform(0.8, 1)
                # if (random_coef == 0):
                #     random_coef = 0.5
                return [new_buyer_covariates[0] * random_coef]

            if len(self.price_history) > 1:
                opponent_alpha = (self.price_history[-2] - last_opponent_price) / self.price_history[-2]
                self.oppo_alphas.append(opponent_alpha)
                if np.mean(self.oppo_alphas[-10:]) > self.lb:
                    self.count_lower += 1
                    if self.count_lower > 50:
                        if not self.is_normal:
                            # if profitable is always being very low
                            return [predicted_price * 0.99]
                        self.is_normal = False
                    return [new_buyer_covariates[0] * 0.95]
                else:
                    self.count_lower = 0
                    self.is_normal = True

            # profitable = (new_buyer_covariates[0] - predicted_price) / new_buyer_covariates[0]
            # if (profitable <= 0.2):
            #     return [new_buyer_covariates[0] * 0.95]
